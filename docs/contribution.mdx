...
* ***Python 3.11 (3.10+ minimum)*** - For pre-commit hooks (and optional custom scripts)

### Recommended Python setup (pyenv)

We recommend [pyenv](https://github.com/pyenv/pyenv) for a consistent Python version on macOS and Linux. Install pyenv following the [installation instructions](https://github.com/pyenv/pyenv#installation), then:

```bash
pyenv install 3.11    # Install Python 3.11
pyenv local 3.11      # Set this repo to use 3.11 (creates .python-version)
python --version      # Verify: should show Python 3.11.x
```

The repo includes a `.python-version` file. If you have pyenv installed, simply run `pyenv install` (if needed) when you clone the repo, and pyenv will automatically use the correct version.
...
Ensure Python 3.10+ (e.g. via pyenv or system). Install and configure pre-commit hooks to ensure code quality:
