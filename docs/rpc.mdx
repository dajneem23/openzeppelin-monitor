...
2. **Network-Specific Transports**:
   ...
   * `SolanaTransportClient` for Solana networks
...
graph TD
    ...
    subgraph Solana Network Calls
        S1[Network Init] -->|getVersion| S2[Process New Blocks]
        S2 -->|getSlot| S3[For each slot]
        S3 -->|getBlock| S4[Process Block]
        S4 -->|For each monitored address| S5[getSignaturesForAddress]
        S5 --> S6[Complete]
    end
...
* RPC Client initialization (per active network): `getVersion`
* Fetching the latest slot (per cron iteration): `getSlot`
* Fetching block data (per block): `getBlock`
* Fetching signatures for monitored addresses: `getSignaturesForAddress`
