...
To test monitors against a local EVM node (no testnet required), see [Local EVM Testing](/monitor/local-evm-testing).
...
* **Tip:** To find the exact event names emitted by a Solana program, inspect any transaction on [Solscan](https://solscan.io) or [Solana Explorer](https://explorer.solana.com) and check the "Program Logs" section to see the log messages and event names
