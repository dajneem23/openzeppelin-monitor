...
* ***Python 3.11 (3.10+)*** - For pre-commit hooks; we recommend [pyenv](https://github.com/pyenv/pyenv) for version management (see [Contribution guidelines](/monitor/contribution#recommended-python-setup-pyenv))
...
1. Check for new Solana blocks every 10 seconds
2. Watch for Deposit events on the Kamino lending program
3. Send notifications via Slack when deposits occur
...
To iterate on monitor behavior without using public testnets, you can run a local EVM node with [Foundry](https://book.getfoundry.sh/) and point the monitor at it. See [Local EVM Testing](/monitor/local-evm-testing) for setup, deployment, and triggering conditions.
